...
const webOverlayDepth: Record<NativeTerminalOverlayMode, number> = {
  opaque: 0,
  "semi-transparent": 0,
};
...
const syncSurface = async (entry: ManagedSurfaceEntry, forceCreate: boolean) => {
  ...
  try {
    ...
    entry.lastSignature = null;
    throw new Error(`Failed to update terminal surface: ${entry.surfaceId}`);
  }
};
...
// Set up global listeners at module load time (browser only)
...
export const terminalSurfaceService = {
  isNativeSupported: () => getNativeSupport().catch(() => false),
  ...
  setAllSurfaceFontSizes: async (fontSize: number): Promise<void> => {
    const supported = await getNativeSupport();
    if (!supported) return;
  ...
  parkSurface: (surfaceId: string): Promise<void> => parkSurfaceEntry(surfaceId),

  removeSurface: (surfaceId: string): Promise<void> => destroySurfaceEntry(surfaceId),

  removeWorkspaceSurfaces: (workspaceId: string): Promise<void> =>
    destroyWorkspaceSurfaces(workspaceId),

  removeAllSurfaces: (): Promise<void> => destroyAllSurfaces(),

  refreshAllSurfaces: async (): Promise<void> => {
    for (const entry of entries.values()) {
      clearScheduledSync(entry);
      if (entry.syncInFlight) {
        entry.pendingCreate = true;
  ...
  beginWebOverlay: async (mode: NativeTerminalOverlayMode): Promise<void> => {
  ...
  endWebOverlay: async (mode: NativeTerminalOverlayMode): Promise<void> => {
  ...
  setWebOcclusionRect: async (id: string, rect: SurfaceRect | null): Promise<void> => {
    if (rect) {
