...
export function ipcGitHeaderBranchContext(workspaceId: string): Promise<HeaderBranchContext> {
  return invoke<HeaderBranchContext>("header_branch_context", { workspaceId });
}

export function ipcGetUiState(key: string): Promise<string | null> {
  return invoke<string | null>("get_ui_state", { key });
}

export function ipcSetUiState(key: string, value: string | null): Promise<void> {
