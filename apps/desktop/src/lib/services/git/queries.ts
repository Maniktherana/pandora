...
import { ipcGitCheckRuns } from "@/lib/services/ipc/client";
import { flattenGitSnapshot, buildGitDecorationIndex } from "./utils";
import type { ScmSnapshot, ScmEntry } from "@/lib/shared/shared.types";
...
export function useCheckRunsQuery(worktreePath: string, options?: GitQueryOptions) {
  return useQuery({
    ...
    queryFn: () => ipcGitCheckRuns(worktreePath),
 ...
 * events.ts pushes a fresh snapshot via setQueryData. Returns cached data
 * when present, undefined on cache miss.
 *
 * Contract:
 * - queryFn is skipToken — never fetches, never starts backend work.
 * - Data is written exclusively by applyGitSnapshot in events.ts.
