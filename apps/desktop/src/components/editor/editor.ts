...
import { defaultTheme, toMonacoTheme } from "@/lib/shared/theme";
import type { WorkspaceTheme } from "@/lib/shared/theme";

export const EDITOR_THEME_ID = "pandora-theme";

/** Editor chrome under Monaco; tracks `applyTheme` via `--theme-code-surface-base`. */
export const EDITOR_BG = "var(--theme-code-surface-base)";

export const EDITOR_FONT_FAMILY = defaultTheme.codeEditor.typography.fontFamily;
export const EDITOR_FONT_SIZE = defaultTheme.codeEditor.typography.fontSize;
export const EDITOR_LINE_HEIGHT = defaultTheme.codeEditor.typography.lineHeight;
...
export function registerEditorTheme(monaco: Monaco, workspaceTheme: WorkspaceTheme): void {
  monaco.editor.defineTheme(EDITOR_THEME_ID, toMonacoTheme(workspaceTheme));
...
export const editorBeforeMount: BeforeMount & DiffBeforeMount = (monaco) => {
  ...
  registerEditorTheme(monaco, defaultTheme);
};
 ...
 */
export function getEditorModelContent(relativePath: string): string | undefined {
