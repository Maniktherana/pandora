...
function measureOcclusionRect(
  ...
  const snapDown = (value: number) => Math.floor(value * 4) / 4;
  const snapUp = (value: number) => Math.ceil(value * 4) / 4;
  const left = snapDown(rect.left - padding);
  const top = snapDown(rect.top - padding);
  const right = snapUp(rect.right + padding);
  const bottom = snapUp(rect.bottom + padding);
...
export function useNativeTerminalOcclusion(
  ...
  const sendRect = useCallback(
    (rect: SurfaceRect | null) => {
      ...
      void terminalSurfaceService.setWebOcclusionRect(id, rect).catch(() => {});
    },
    [id],
  );
