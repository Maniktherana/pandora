...
export class Terminal implements ITerminalCore {
  ...
  private startRenderLoop(): void {
    ...
    const loop = () => {
      if (!this.isDisposed && this.isOpen) {
        ...
        // Update mouse cursor: use default arrow when app has mouse tracking
        // (btop, vim, etc.), text cursor otherwise
        if (this.canvas) {
          const wantDefault = this.wasmTerm!.hasMouseTracking();
          const current = this.canvas.style.cursor;
          // Don't override 'pointer' (set by link hover)
          if (current !== 'pointer') {
            const desired = wantDefault ? 'default' : 'text';
            if (current !== desired) {
              this.canvas.style.cursor = desired;
              if (this.element) this.element.style.cursor = desired;
            }
          }
        }
    ...
    this.linkDetector
      ...
      .then((link) => {
        ...
        if (link !== this.currentHoveredLink) {
          ...
          const defaultCursor = this.wasmTerm?.hasMouseTracking() ? 'default' : 'text';
          const cursorStyle = link ? 'pointer' : defaultCursor;
          if (this.element) {
  ...
  private handleMouseLeave = (): void => {
    ...
    if (this.currentHoveredLink) {
      ...
      // Reset cursor — render loop will set text vs default based on mouse tracking
      const resetCursor = this.wasmTerm?.hasMouseTracking() ? 'default' : 'text';
      if (this.element) {
        this.element.style.cursor = resetCursor;
        if (this.canvas) {
          this.canvas.style.cursor = resetCursor;
        }
      }
    }
  };
