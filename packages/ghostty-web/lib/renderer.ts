...
export class CanvasRenderer {
  ...
  public render(
    ...
    // Scroll-blit optimization: when viewport changes by N lines, copy
    // existing content up/down by N line-heights and only render the
    // newly exposed lines. This avoids full-canvas redraws during scrolling.
    const viewportLine = Math.floor(viewportY);
    const lastViewportLine = Math.floor(this.lastViewportY);
    const scrollDelta = viewportLine - lastViewportLine; // positive = scrolled up more
    this.lastViewportY = viewportY;

    if (scrollDelta !== 0 && !forceAll && Math.abs(scrollDelta) < dims.rows) {
      // Blit: shift existing canvas content by scrollDelta lines
      const shiftPx = scrollDelta * this.metrics.height * this.devicePixelRatio;
      // drawImage self-copy: shift content down (scrollDelta > 0) or up (scrollDelta < 0)
      this.ctx.save();
      this.ctx.setTransform(1, 0, 0, 1, 0, 0); // reset DPI scale for raw pixel copy
      this.ctx.drawImage(this.canvas, 0, shiftPx);
      this.ctx.restore();
      // Re-apply DPI scale after self-copy reset it
      this.ctx.setTransform(this.devicePixelRatio, 0, 0, this.devicePixelRatio, 0, 0);

      // Only render the newly exposed lines
      if (scrollDelta > 0) {
        // Scrolled up: new lines at top (rows 0..scrollDelta-1)
        for (let y = 0; y < scrollDelta; y++) {
          const line = this.getLineForViewport(y, viewportLine, scrollbackLength, scrollbackProvider, buffer);
          if (line) {
            this.renderLine(line, y, dims.cols);
          } else {
            this.ctx.fillStyle = this.theme.background;
            this.ctx.fillRect(0, y * this.metrics.height, dims.cols * this.metrics.width, this.metrics.height);
          }
        }
      } else {
        // Scrolled down: new lines at bottom
        for (let y = dims.rows + scrollDelta; y < dims.rows; y++) {
          const line = this.getLineForViewport(y, viewportLine, scrollbackLength, scrollbackProvider, buffer);
          if (line) {
            this.renderLine(line, y, dims.cols);
          } else {
      ...
      // Render cursor and scrollbar on top
    ...
    if (scrollDelta !== 0) {
      forceAll = true; // Large scroll or first scroll — full redraw
    }
  ...
  /**
   * Get line content for a given canvas row at a viewport position.
   * Fetches from scrollback or screen buffer as appropriate.
   */
  private getLineForViewport(
    y: number,
    viewportLine: number,
    ...
    if (viewportLine > 0) {
      if (y < viewportLine && scrollbackProvider) {
