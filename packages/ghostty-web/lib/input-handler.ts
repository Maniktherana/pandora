...
export class InputHandler {
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    // Returns true = "app handled it, terminal should skip" (no preventDefault —
    // the app-level handler decides whether to prevent default, e.g. Cmd+Q must
    // NOT be prevented so the OS can quit the app).
    ...
    // macOS keybindings: Option+Arrow for word movement, Option+Backspace for word delete
    // The WASM key encoder doesn't replicate native Ghostty's macOS-specific Option key handling.
    if (event.altKey && !event.ctrlKey && !event.metaKey && !event.shiftKey) {
      let macSeq: string | null = null;
      switch (key) {
        case Key.LEFT:  macSeq = '\x1bb'; break; // ESC b — word back
        case Key.RIGHT: macSeq = '\x1bf'; break; // ESC f — word forward
        case Key.BACKSPACE: macSeq = '\x1b\x7f'; break; // ESC DEL — delete word back
      }
      if (macSeq !== null) {
        event.preventDefault();
        event.stopPropagation();
        this.onDataCallback(macSeq);
        this.recordKeyDownData(macSeq);
        return;
      }
    }
